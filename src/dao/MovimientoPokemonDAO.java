...
public class MovimientoPokemonDAO {
	...
	//he hecho lo mismo que en movimientoDAO porque no sé que se hace en este asi que cntl+c cntl+v
    public static LinkedList<Movimiento> cargarTodos(Connection con, MovimientoPokemonDAO movimientoPokemonDAO) {
    ...
    public static Movimiento buscarPorId(Connection con, int id) {
        ...
        try (PreparedStatement ps = con.prepareStatement(query)) {
            ...
            if (rs.next()) {
                m = new Movimiento(query, id, id, id, id, query, id, query, query, id, query, id);
                m.setIdMovimiento(rs.getInt("ID_MOVIMIENTO"));
                m.setNombre(rs.getString("NOM_MOVIMIENTO"));
                m.setNivelAprendizaje(rs.getInt("NIVEL_APRENDIZAJE"));
                m.setPpMax(rs.getInt("PP_MAX"));
